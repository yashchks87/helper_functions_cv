...
def compile_new_model(models, app_type, name, strategy, label_smoothing=None, learning_rate=None):
    length = len(models)
    with strategy.scope():
        model = get_model(models=models, length=length, app_type=app_type, name=name)
        if label_smoothing:
            losses = [tf.keras.losses.BinaryCrossentropy(label_smoothing=label_smoothing) for i in range(length)]
        else:
