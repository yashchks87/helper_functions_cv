...
class GetMatrics():
    def __init__(self, truth, preds, positive = 1, negative = 0):
        ...
        self.TP = np.sum(np.logical_and(self.truth == self.positive, self.preds == self.positive))
        # This will calculate all true negatives
        self.TN = np.sum(np.logical_and(self.truth == self.negative, self.preds == self.negative))
        # This will calculate all false positives
        self.FP = np.sum(np.logical_and(self.truth == self.negative, self.preds == self.positive))
        # This will calculate all false negatives
        self.FN = np.sum(np.logical_and(self.truth == self.positive, self.preds == self.negative))
